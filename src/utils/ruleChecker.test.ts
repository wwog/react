...
describe('ruleChecker', () => {
  ...
  describe('基础功能', () => {
    ...
    it('应该验证自定义 validator - 返回 string', () => {
      ...
      const rules: RuleDescription<typeof data> = {
        username: {
          validator: (value) => (value === 'admin' ? '用户名不能是admin' : true),
        },
      }
      const result = ruleChecker(data, rules)
      expect(result.valid).toBe(false)
  ...
  describe('数组规则', () => {
    ...
    it('应该验证数组元素规则', () => {
      ...
      if (!result.valid) {
        // 检查错误信息包含了数组元素索引的错误
        expect(result.errors.some((error) => error.includes('emails[1]'))).toBe(true)
      }
    })
  })
  ...
  describe('依赖验证', () => {
    ...
    it('应该验证 dependsOn - 返回 string', () => {
      ...
      const rules: RuleDescription<typeof data> = {
        hasLicense: {
          dependsOn: (data) => ((data.age || 0) < 18 ? '未成年人不能有驾驶证' : true),
        },
      }
      const result = ruleChecker(data, rules)
      expect(result.valid).toBe(false)
  ...
  describe('复杂场景', () => {
    ...
    it('应该处理复杂的用户注册场景', () => {
      ...
      const rules: RuleDescription<typeof data> = {
        ...
        password: [{required: true}, {min: 6}, {regex: /[A-Z]/, message: '密码必须包含大写字母'}],
        confirmPassword: {
          required: true,
      ...
      expect(result.valid).toBe(false)
      if (!result.valid) {
        expect(result.fieldErrors.username).toContain('username 长度不能少于 3')
        expect(result.fieldErrors.password).toContain('password 长度不能少于 6')
